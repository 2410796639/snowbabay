...
void subwidget::timerEvent(QTimerEvent *)
{
    ...
    for(int i=0;i<100;i++)
    {
        for(int j=0;j<pigcage[i].pigsum;j++)
        {
    ...
    for(int i=0;i<100;i++)
    {
        if(pigcage[i].infection==false)
        {
            continue;
        }else if(pigcage[i].infection==true)
        {
            for(int j=0;j<pigcage[i].pigsum;j++)
            {
            ...
            if(i==0)
            {
                ...
                {
                    continue;
                }else if(pigcage[i+1].infection==false)
                {

                    for(int k=0;k<pigcage[i+1].pigsum;k++)
                    {
                        judge=qrand()%100;
                        if(judge<15)
                        {
                            badpignumber++;
                            pigcage[i+1].infection=true;
                            pigcage[i+1].pigpen[k].infection=true;
                            pigcage[i+1].pigpen[k].infec="是";
                        }
                    }
                }
            }else if(i==99)
    ...
    ui->label_4->setText(QString("感染猪瘟的猪有%1只").arg(badpignumber));
    //一个月结束进行卖猪与自动补充
    if(sec%90==0&&sec!=0)
    {
        int bsell=0;
